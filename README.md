...
| 틀린 83장 중 미성숙 과립구가 낀 것 | **83%** |
| **서로 다른 네 모델이 똑같이 틀린 장** | **27장** |
| 그게 우연히 겹칠 기댓값 | 0.00장 |

시드·해상도·구조를 바꿔 학습한 네 모델이 **같은 27장**을 틀리고, 26장은 **같은
답으로**, 15장은 **넷 다 90% 이상 확신**하며 틀린다. 학습의 우연으로는 설명되지
않는다 — 원인은 그 이미지들에 있다.
...
- **네 모델이 ImageNet 사전학습을 공유한다.** 마지막 공통점이라 완전히
  배제하지 못했다.
