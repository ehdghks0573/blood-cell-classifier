...
**남은 14장을 224px 로 눈으로 보면 더 분명하다.** 호중구로 라벨된 6장과 미성숙
과립구로 라벨된 5장을 나란히 놓으면 **서로 구분되지 않는다** — 양쪽 다 핵이
오목하게 패이거나 두 덩이로 붙은 중간 모습이고, 어느 쪽도 자기 라벨의 대표를
닮지 않았다. 한 방향만 보면 "모델이 한쪽으로 기울었다"로 설명되지만, **양방향이
같은 자리에 모여 있으면** 그 설명은 성립하지 않는다.
...
#### torch 2.5.1 에 고정해 둔 이유 (09-18)

GitHub 이 이 저장소에 Dependabot 경고 9건을 띄운다. **9건 모두 torch 하나**를
가리키고, 그중 하나(`torch.load` 의 `weights_only=True` 우회, 2.6.0 에서 수정)가
critical 이다.

**그래도 올리지 않는다.** 이 프로젝트의 결론 하나가 "같은 시드·같은 명령이면
소수점 15자리까지 같다"(`docs/results.md` 4절)인데, torch 를 올리면 그 숫자들이
전부 다시 재야 하는 값이 된다. 다른 GPU 로 옮겼을 때 EfficientNet-B0 이 0.0114
어긋난 전례가 있어(`docs/experiments.md` 09-17), 버전 변경이 무해하다고 가정할
수 없다.

**대신 위험을 없앴다.** 경고들은 전부 *공격자가 만든 입력*을 넘길 때의 이야기다.
이 저장소가 `torch.load` 로 읽는 것은 자기가 만든 `runs/*/best.pt` 뿐이고,
그 안에는 텐서·문자열·숫자만 들어 있다. 그래서 세 군데의 `torch.load` 를 모두
`weights_only=True` 로 바꿨다 — 경로상 임의 코드 실행이 일어날 여지를 닫는다.
**남의 체크포인트를 이 코드로 열지 말 것.** 그때는 torch 부터 올려야 한다.
...
tools/
  ...
  boundary.py         라벨이 반대인 두 무리를 나란히 놓는다
